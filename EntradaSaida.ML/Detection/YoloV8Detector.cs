...
namespace EntradaSaida.ML.Detection
{
    ...
    public class YoloV8Detector : IDetectionService, IDisposable
    {
        ...
        public async Task<List<Core.Models.Detection>> DetectPersonsAsync(byte[] imageData)
        {
            return await DetectPersonsAsync(imageData, DefaultConfidenceThreshold);
        }
    
        public async Task<List<Core.Models.Detection>> DetectPersonsAsync(byte[] imageData, float confidenceThreshold)
        {
            if (!IsModelLoaded)
            ...
            try
            {
                ...
                var personDetections = detectionResults
                    ...
                    .Select(d => new Core.Models.Detection
                    {
