...
namespace EntradaSaida.ML.Processing
{
    ...
    public class FrameProcessor : IDisposable
    {
        ...
        /// </summary>
        private async Task<byte[]> CreateAnnotatedFrameAsync(byte[] frameData, List<Core.Models.Detection> detections, List<TrackedObject> tracks)
        {
            try
            {
                using var mat = new Mat();
                CvInvoke.Imdecode(frameData, ImreadModes.AnyColor, mat);
